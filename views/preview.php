...
$preview .= '</div>';
$preview .= '<div class="tc-text"><p>' . esc_html__( 'The card for your website will look a little something like this!', 'jm-tc' ) . '</p> </div>';

$card  = '<div class="tc-container">';
$card .= '<img class="tc-img ' . esc_attr( $type ) . '-img" src="' . esc_url( $image ) . '" alt="">';
$card .= '<div class="' . esc_attr( $type ) . '-content"><h2 class="' . esc_attr( $type ) . '-title tc-title">' . esc_html( $title ) . '</h2>
          <p class="' . esc_attr( $type ) . '-desc tc-desc">' . esc_html( $desc ) . '</p>
          <span class="tc-domain ' . esc_attr( $type ) . '-domain">' . _( $domain ) . '</span></div></div>';
