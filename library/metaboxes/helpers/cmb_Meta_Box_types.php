...
class cmb_Meta_Box_types {
	...
	public function render_repeatable_field() {
		...
		<p class="add-row">
			<a data-selector="<?php echo $table_id; ?>" class="add-row-button button"
			   href="#"><?php _e( 'Add Row', 'cmb' ); ?></a>
		</p>
	...
	public function text_url() {
		return $this->input( array(
			'class' => 'cmb_text_url cmb_text_medium regular-text',
	...
	public function text_money() {
		return ( ! $this->field->args( 'before' ) ? '$ ' : ' ' ) . $this->input( array(
			'class' => 'cmb_text_money',
			'desc'  => $this->_desc()
		) );
	}
	...
	public function textarea( $args = array() ) {
		...
		return sprintf( '<textarea%s>%s</textarea>%s', $this->concat_attrs( $args, array(
			'desc',
			'value'
		) ), $args['value'], $args['desc'] );
	}
	...
	public function multicheck( $type = 'checkbox' ) {
		return $this->radio( array(
			'class'   => 'cmb_checkbox_list cmb_list',
			'options' => $this->concat_options( array(
				'type' => 'checkbox',
				'name' => $this->_name() . '[]'
			), 'list_input_checkbox' )
		), $type );
